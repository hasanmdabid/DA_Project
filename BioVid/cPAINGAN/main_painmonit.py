...
# This sript will train the conditional GAN model on PainMonit datasets. The general instruction for the users are given below:
# 1st step: Please read the dataset from the following folder "/home/abidhasan/Documents/DA_Project/BioVid/datasets/"
# 2nd step: Run the sript. It will train the model and save the results in the following folder "/home/abidhasan/Documents/DA_Project/BioVid/PainGan/results"
# 3rd step: The trained model will  
# 4th step: To generate the synthetic data using the trained cGAN model, rub the "Synthetic_data_Generator.py" script. The script will use the trined GAN 
# model to generate the synthetic data and save them in the following folder "/home/abidhasan/Documents/DA_Project/BioVid/PainGan/Data/cGAN_Generated_data". 
# User can use this data as cGAN synthetic Generated data to evalauate the results.
...
folder_path = './Data'
dataset_name = 'painmonit'
if dataset_name == 'biovid':
    data_file_name = 'X.npy'
    label_file_name = 'y.npy'
elif dataset_name == 'painmonit':
    data_file_name = 'X.npy'
    label_file_name = 'y.npy'
# Combine folder path and file names
data_full_path = f'{folder_path}/{dataset_name}/{data_file_name}'
label_full_path = f'{folder_path}/{dataset_name}/{label_file_name}'
...
save_pic_dir = '/home/abidhasan/Documents/PAINGAN/picture'
save_model_dir = '/home/abidhasan/Documents/PAINGAN/results/trained_model'
save_loss_dir = '/home/abidhasan/Documents/PAINGAN/results/loss_record'
...
dump(scaler, f'{dataset_name}_minmax_scaler.bin', compress=True)
...
def define_discriminator(in_shape=(2560, 1, 1), n_classes=2):
    # label input
    in_label = Input(shape=(1,), name='Discriminator-Label-Input-Layer')  # Shape 1
    # embedding for categorical input
    # each label (total 2 classes for PAINMONIT), will be represented by a vector of size 200.
    # This vector of size 200 will be learnt by the discriminator
    li = Embedding(n_classes, 200, name='Discriminator-Label-Embedding-Layer')(in_label)  # Shape 1,200
    # scale up to image dimensions with linear activation
    n_nodes = in_shape[0] * in_shape[1]  # 2560*1 = 2560.
    li = Dense(n_nodes, name='Discriminator-Label-Dense-Layer')(li)  # Shape = 1, 2560
    #li = BatchNormalization(momentum=0.4)(li)
    # reshape to additional channel
    li = Reshape((in_shape[0], in_shape[1], 1), name='Discriminator-Label-Reshape-Layer')(li)  # 2560x1x1
...
plot_model(dis_model, show_shapes=True, show_layer_names=True, dpi=400, to_file=f'/home/abidhasan/Documents/PAINGAN/picture/{dataset_name}_discriminator_structure.png')
...
def define_generator(latent_dim, n_classes=2):
    ...
    li = Embedding(n_classes, 200, name='Generator-Label-Embedding-Layer')(in_label)  # Shape 1,50

    # linear multiplication
    n_nodes = 80 * 1  # To match the dimensions for concatenation later in this step.
    li = Dense(n_nodes, name='Generator-Label-Dense-Layer')(li)  # 1,64
    # reshape to additional channel
    li = Reshape((80, 1, 1), name='Generator-Label-Reshape-Layer')(li)
    ...
    n_nodes = 80 * 1 * 2  # This Part is very Important
    ...
    gen = Reshape((80, 1, 2), name='Generator-Foundation-Layer-Reshape-1')(gen)  # Shape=8x40x2

    # merge image gen and label input
    merge = Concatenate(name='Generator-Combine-Layer')([gen, li])  # Shape=8x40x3 (Extra channel corresponds to the label)

    # up-sample to 16x16 ====== HIDDEN layer 1
    gen = Conv2DTranspose(filters=64, kernel_size=(4, 1), strides=(2, 1), padding='same', name='Generator-Hidden-Layer-1')(merge)  # 16x40x2
    ...
    # up-sample to 16x16 ====== HIDDEN layer 1
    gen = Conv2DTranspose(filters=64, kernel_size=(4, 1), strides=(2, 1), padding='same', name='Generator-Hidden-Layer-2')(gen)  # 32x40x64
    #gen = BatchNormalization(momentum=0.4)(gen)
    ...
    gen = Dropout(0.4, name='Generator-2ndCONV2D-Layer-Dropout')(gen)
        ...
        # up-sample to 16x16 ====== HIDDEN layer 1
    gen = Conv2DTranspose(filters=64, kernel_size=(4, 1), strides=(2, 1), padding='same', name='Generator-Hidden-Layer-3')(gen)  # 64x40x64
    #gen = BatchNormalization(momentum=0.4)(gen)
    ...
    gen = Conv2DTranspose(filters=32, kernel_size=(4, 1), strides=(2, 1), padding='same', name='Generator-Hidden-Layer-4')(gen)  # 128x40x32
    #gen = BatchNormalization(momentum=0.4)(gen)
    ...
    gen = Dropout(0.4, name='Generator-4thCONV2D-Layer-Dropout')(gen)
        ...
        # up-sample to 32x32 ========== HIDDEN layer 1
    gen = Conv2DTranspose(filters=32, kernel_size=(4, 1), strides=(2, 1), padding='same', name='Generator-Hidden-Layer-5')(gen)  # 128x40x32
    #gen = BatchNormalization(momentum=0.4)(gen)
    gen = LeakyReLU(alpha=0.2, name='Generator-Hidden-Layer-Activation-5')(gen)
    gen = Dropout(0.4, name='Generator-5thCONV2D-Layer-Dropout')(gen)

    # output
    out_layer = Conv2D(filters=1, kernel_size=(3, 1), strides =(1,1), activation='tanh', padding='same', name='Generator-Output-Layer')(gen)  # 128x40x1
    # define model
...
# Show model summary and plot model diagram
plot_model(gen_model, show_shapes=True, show_layer_names=True, dpi=400, to_file=f'/home/abidhasan/Documents/PAINGAN/picture/{dataset_name}_generator_structure.png')
...
def saveResultsCSV(gen_activation, epochs, batch_per_epoch, d_loss_real, d_loss_fake, g_loss):
    ...
    fileString = save_loss_dir + f'/{dataset_name}_D_and_G_loss.csv'
...
def train(g_model, d_model, gan_model, dataset, latent_dim, n_epochs, n_batch):
    ...
    g_model.save(save_model_dir + f'/CGAN_PAIN_{dataset_name}_{n_epochs}epochs_{n_batch}Batch.h5')
