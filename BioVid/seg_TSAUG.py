...
def seg_TSAUG(x, y, Aug_factor):
    ...
    y = np.repeat(y,L, axis=0)  # Converting the labels into total number of time stamp
    y = get_strides(y, L, ov)      # Changing shape from 2D to 3D
    
    # -----------------Creating the augmenter class---------------------------------------------------
    timewarp_augmenter = (TimeWarp() * Aug_factor)  # random time warping 16 times in parallel)
    cropping_augmenter = (Crop(size=L) * Aug_factor)  # random crop subsequences with length 16)
