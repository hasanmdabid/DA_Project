...
class rf(classifier):
	...
	def train(self):
		...
		if "rfe_manual" in self.param and self.param["rfe_manual"] != None and type(self.param["rfe_manual"]) == int:
			model = RandomForestRegressor(n_estimators = self.param["n_estimators"], max_depth= self.param["max_depth"], min_samples_split= self.param["min_samples_split"])
		...
		self.model = RandomForestRegressor(n_estimators = self.param["n_estimators"], max_depth= self.param["max_depth"], min_samples_split= self.param["min_samples_split"])
