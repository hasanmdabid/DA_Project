...
def seg_TSAUG(data):
    ...
    if priority == 'YES':
        ...
        time_augmenter = (TimeWarp() * factor)  # random time warping 16 times in parallel)
        ...
        x_1st_aug, y_1st_aug = crop_augmenter.augment(x_min, y_min)
        #x_2nd_aug, y_2nd_aug = jittering_augmenter.augment(x_min, y_min)

        """ 
        print('After time Augmentation of Min clas only:')
        print('x_time_aug:', x_time_aug.shape)
        print('y_time_aug:', y_time_aug.shape)
        unique_time_aug, counts_time_aug = np.unique(y_time_aug, return_counts=True)
        print('After Augmentation: Classes and Instances')
        print(np.asarray((unique_time_aug, counts_time_aug)).T)

        print('After crop Augmentation of Min clas only:')
        print('x_crop_aug:', x_time_aug.shape)
        print('y_crop_aug:', y_time_aug.shape)
        unique_crop_aug, counts_crop_aug = np.unique(y_crop_aug, return_counts=True)
        print('After Augmentation: Classes and Instances')
        print(np.asarray((unique_crop_aug, counts_crop_aug)).T)

        print('x_maj:', x_maj.shape)
        print('x_min:', x_min.shape)
        print('x_time_aug:', x_time_aug.shape)
        print('x_crop_aug:', x_crop_aug.shape)
        """

        x = np.concatenate((x_maj, x_min, x_1st_aug), axis=0)
        y = np.concatenate((y_maj, y_min, y_1st_aug), axis=0)
